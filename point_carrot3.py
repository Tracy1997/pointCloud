...
def findpcd(rgb_img,depth_img,orientation,position, pinhole_camera_intrinsic, apply_mask):
	...
	#o3d.visualization.draw_geometries([pcd_robot_carrot])
...
# cv2.imwrite('color_carrot4.jpg', data["rgb_images"][4])
# cv2.imwrite('color_carrot5.jpg', data["rgb_images"][5])
...
pcd4full, pcd4rc, pcd4 = findpcd(data["rgb_images"][4],data["depth_images"][4],data["ee_orientation"][4], data["ee_position"][4], pinhole_cam_intrinsic, False)
pcd5full, pcd5rc, pcd5 = findpcd(data["rgb_images"][5],data["depth_images"][5],data["ee_orientation"][5], data["ee_position"][5], pinhole_cam_intrinsic, False)
...
# np_colors = np.array(pcd4.colors)
# np_colors[:,1] = 1
# pcd3.colors = o3d.utility.Vector3dVector(np_colors)

c_pcds = [pcd0, pcd1, pcd2, pcd3, pcd4, pcd5]
rc_pcds = [pcd0rc, pcd1rc, pcd2rc, pcd3rc, pcd4rc, pcd5rc]
full_pcds = [pcd0full, pcd1full, pcd2full, pcd3full, pcd4full, pcd5full]
...
threshold = 0.05
trans_init = np.eye(4)
reg_p2p_translation = o3d.pipelines.registration.registration_icp(
	pcd_rc, base, threshold, trans_init,
	o3d.pipelines.registration.TransformationEstimationPointToPoint(),
	o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=200))

pcd_rc.transform(reg_p2p_translation.transformation)
pcd_c.transform(reg_p2p_translation.transformation)

#o3d.visualization.draw_geometries([pcd0rc, pcd1rc])
#o3d.visualization.draw_geometries([pcd0, pcd1])

cam2w = sylvester(-AtoB, np.linalg.inv(reg_p2p_translation.transformation), np.zeros((4,4),dtype=float))
cam_frame_true = CoordinateFrame(world_frame_3D, R.from_matrix(cam2w[0:3,0:3]).inv(), cam2w[-1][0:3])  # this is the default
...
for i in [0,2,3,4,5]:
	...
	if(i==0):	
		pcd_from_a_to_b(c_pcds[1], cam_frame_i, world_frame_3D)
		pcd_from_a_to_b(full_pcds[1], cam_frame_i, world_frame_3D)
...
#o3d.visualization.draw_geometries([pcd0, pcd1, pcd2, pcd3, pcd4, pcd5])
...
mesh_cylinder = o3d.geometry.TriangleMesh.create_cylinder(radius=pcd0extent[dim_order[1]]/2,height=pcd0extent[dim_order[2]],resolution=20, split=80)
r=np.array([[0, 0, 1],[1, 0, 0],[0, 1, 0]])
mesh_cylinder.rotate(r)
mesh_cylinder.rotate(pcd02world)
mesh_cylinder.translate(pcd0.get_center()-mesh_cylinder.get_center())
...
o3d.visualization.draw_geometries([pcd_combined, pcd0_oriented_bounding_box,aligned_bounding_box, oriented_bounding_box])
alpha = 0.5
